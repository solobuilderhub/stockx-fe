...
const popularRestaurants = [
  { id: "r1", name: "KFC", cuisine: "Fast Food • American", image: null, rating: 4.5, deliveryTime: "15-25 min" },
  { id: "r2", name: "Popeyes", cuisine: "American • Chicken", image: null, rating: 4.2, deliveryTime: "20-30 min" },
  { id: "r3", name: "Spice Delight", cuisine: "Indian • Vegetarian", image: null, rating: 4.8, deliveryTime: "25-35 min" }
];
...
const ExpandedSearch = ({ className, onCollapse, isDesktop = false }) => {
  ...
  const getSuggestions = (query) => {
    ...
    setTimeout(() => {
      ...
      const mockItems = query.length > 0 
        ? [
            { type: "category", name: "Categories", items: searchCategories.filter(cat => 
              cat.name.toLowerCase().includes(query.toLowerCase())
            ).slice(0, 3) },
            { type: "restaurant", name: "Restaurants", items: popularRestaurants.filter(item => 
              item.name.toLowerCase().includes(query.toLowerCase()) || 
              item.cuisine.toLowerCase().includes(query.toLowerCase())
            ) },
            { type: "dish", name: "Popular Dishes", items: [
              { id: "d1", name: "Butter Chicken", restaurant: "Spice Delight", image: "https://images.unsplash.com/photo-1588166524941-3bf61a9c41db?q=80&w=684&auto=format&fit=crop" },
              { id: "d2", name: "Veggie Biryani", restaurant: "Curry House", image: "https://images.unsplash.com/photo-1589302168068-964664d93dc0?q=80&w=687&auto=format&fit=crop" }
            ].filter(item => 
              item.name.toLowerCase().includes(query.toLowerCase()) || 
        ...
        : [
            { type: "category", name: "Categories", items: searchCategories.slice(0, 5) },
            { type: "restaurant", name: "Popular Near You", items: popularRestaurants },
            { type: "recent", name: "Recent Searches", items: [
              "Chicken Biryani", "Halal", "Vegetarian"
            ]}
          ];
      
      setSuggestions(mockItems.filter(section => section.items.length > 0));
      setLoading(false);
    }, 300);
  };
        ...
        <div className="search-header p-3 px-4 border-b flex items-center space-x-3 bg-gray-100">
          ...
          <div className="flex-1 flex items-center">
            <input
              ...
              placeholder="Search Halal Kitchen"
  ...
  return (
    <div className={cn("fixed inset-0 bg-white z-50 flex flex-col", className)}>
      {/* Search Header */}
      <div className="search-header p-3 px-4 border-b flex items-center space-x-3 bg-gray-100">
        <ChevronLeft 
          className="h-5 w-5 text-gray-500 cursor-pointer" 
      ...
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-sm text-gray-600">{selectedLocation?.name || "Select a location"}</p>
        </div>
      </button>
  ...
  function renderSectionContent(section, isMobile = false) {
    ...
    switch(section.type) {
      case "category":
        return (
          <div className={`${isMobile ? 'px-4' : 'px-2'} overflow-x-auto scrollbar-hide`}>
