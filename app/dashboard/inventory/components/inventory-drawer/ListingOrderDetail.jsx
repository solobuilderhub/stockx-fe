...
export function ListingOrderDetail({ order, platform, onClose }) {
    ...
    const getSize = () => {
        ...
        if (order && order.size && order.size_unit) {
            return `${order.size} ${String(order.size_unit).replace(
                "SIZE_UNIT_",
                ""
            )}`;
        }
    ...
    return (
        <Card className="w-full">
                        ...
                        <div className="space-y-1">
                            ...
                            {isStockXOrder(order) && (
                                <div className="flex items-center gap-2 text-sm">
                                    ...
                                    />
                                    <span>
                                        Style ID: {order.product.styleId}
                                    </span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-secondary/10 p-4 rounded-md space-y-3">
                            ...
                            {tracking.shipByDate && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>
                                        Ship By:{" "}
                                        {formatDate(tracking.shipByDate)}
                                    </span>
                                </div>
                            )}
            ...
            <CardFooter className="border-t pt-4 flex justify-between">
                ...
                {isStockXOrder(order) &&
                    order.shipment &&
                    order.shipment.shippingLabelUrl && (
                        <Button variant="secondary" className="gap-2" asChild>
                            <a
                                href={order.shipment.shippingLabelUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                            >
                                <FileText size={16} />
